...
// @downloadURL  http://github.com/Xavi-LR/line-rider-mods-and-tools/raw/main/fill-tool.user.js
// @updateURL    http://github.com/Xavi-LR/line-rider-mods-and-tools/raw/main/fill-tool.user.js
...
(function () {
  ...
  const fillToolDefaults = {
    ...
    // Extra stuff
    numLayers: getSimulatorLayers(window.store.getState()).length,
    loadedSettings: true
  };
  ...
  // }
    ...
    function delay(time) { // i use this to make selected lines actually render when switching tools but there's definitely a better way
        ...
        if (!matched) {
          ...
          if (window.fillToolState.prioritizeConnected) {
            if (d1 < 0.001) {
              ...
              continue; // this makes it never get to tolerance test i think, which is bad
            }
