...
function main() {
  ...
  class SmoothPencilTool extends DefaultTool {
    ...
    _chooseLayerByAccretion(folderLayerIds, x1, y1, x2, y2, width) {
      try {
        if (!Array.isArray(folderLayerIds) || folderLayerIds.length === 0) return null;
        ...
        if (
          !state ||
          !state.simulator ||
          !state.simulator.engine ||
          typeof state.simulator.engine.selectLinesInRadius !== "function"
        ) {
          return null;
        }
        ...
        const nearby = engine.selectLinesInRadius({ x: midX, y: midY }, width) || [];

        const present = new Array(folderLayerIds.length).fill(false);
        for (const L of nearby) {
          if (!L || L.layer == null) continue;
          const idx = folderLayerIds.indexOf(L.layer);
        ...
        if (highestValidIdx < 0) return null;

        let desiredIdx =
            highestValidIdx >= folderLayerIds.length - 1
        ? highestValidIdx
        : highestValidIdx + 1;

        if (typeof this._lastGeneratedFolderIdx === "number") {
          const minAllowed = Math.max(0, this._lastGeneratedFolderIdx - 2);
          if (desiredIdx < minAllowed) desiredIdx = minAllowed;
        }

        desiredIdx = Math.max(0, Math.min(folderLayerIds.length - 1, desiredIdx));

        this._lastGeneratedFolderIdx = desiredIdx;
        return folderLayerIds[desiredIdx];
      } catch (e) {
        return null;
      }
    }
    ...
    _generateCrayonDots(x1, y1, x2, y2, dotsCount, dotWBase, spread, thicknessVariation, mult, type, flipped, multicolored, folderLayerIds, multicolorMode, randomColor, boomerang, pressureVal, dotLengthMul, midBias) {
      ...
      for (let i = 0; i < dotsCount; i++) {
        ...
        if (multicolored) {
          let lid = null;
          if (multicolorMode === "accretion") {
            lid = this._chooseLayerByAccretion(folderLayerIds, sx, sy, ex, ey, dotW);
          } else {
            lid = this._chooseLayer(folderLayerIds, multicolored, multicolorMode, randomColor, boomerang, pressureVal, perCallLayer);
          }
    ...
    _makeLineObjLiteral(x1, y1, x2, y2, w, m, t, flipped, layerId, folderLayerIds, multicolored, multicolorMode, randomColor, boomerang, pressureVal) {
      ...
      if (typeof layerId !== 'undefined' && layerId !== null) {
        ...
        if (multicolorMode === "accretion") {
          accAssigned = this._chooseLayerByAccretion(
            folderLayerIds,
            x1, y1, x2, y2,
            w
          )
      ...
      try {
        if (multicolored && multicolorMode === "accretion") {
          const folderIds = Array.isArray(folderLayerIds) && folderLayerIds.length ? folderLayerIds.slice(0) : getFolderLayerIds();
          const idx = folderIds.indexOf(assignedLayer)
          if (idx >= 0) this._lastGeneratedFolderIdx = idx
        }
      } catch (e) {}
    ...
    _generateBrushStrokes(x1, y1, x2, y2, count, spread, thicknessJitter, bristleThickness, mult, type, flipped, folderLayerIds, multicolored, multicolorMode, randomColor, boomerang, pressureVal) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (multicolored) {
          let lid = null;
          if (multicolorMode === "accretion") {
            lid = this._chooseLayerByAccretion(folderLayerIds, sx, sy, ex, ey, w);
          } else {
            lid = this._chooseLayer(folderLayerIds, multicolored, multicolorMode, randomColor, boomerang, pressureVal, perCallLayer)
          }
          if (lid != null) {
      ...
      return e("div", { key: "smooth-pencil-root" }, [
        ...
        this.state.customizeOpen && e("div", { style: this.sectionBox }, [
          ...
          this.state.multicolored ? e("div", { style: this.sectionBox }, [
            ...
            this.renderRadio("multicolorMode", "accretion", "Accretion"),
